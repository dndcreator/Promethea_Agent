    ...
    if request.action == "approve":
        ...
        try:
            ...
            if not all_tool_calls:
                # 兼容旧数据（虽然理论上不会有）
                all_tool_calls = [{'name': pending['tool_name'], 'args': pending['args'], 'id': pending['tool_call_id']}]
            ...
            # 检查是否是新的确认请求（链式确认）
            # 注意：execute_tool_calls 可能会再次抛出 ToolConfirmationRequired
            # 但我们需要在 tool_call_loop 外面捕获它吗？
            # 不，execute_tool_calls 直接抛出异常。
            # 如果是 ToolConfirmationRequired，我们需要捕获它并返回新的确认响应。
            
            # 由于 ToolConfirmationRequired 是在 agentkit.mcp.tool_call 定义的，我们需要导入它
        ...
        tool_call_outcome = await state.conversation.run_chat_loop(messages, user_config, session_id=session_id)
        ...
        return ChatResponse(response=final_content, session_id=session_id, status="success")
