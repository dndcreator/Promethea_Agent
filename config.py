...
class Neo4jConfig(BaseModel):
    ...
    uri: str = Field(default_factory=lambda: os.getenv("NEO4J_URI", "bolt://localhost:7687"), description='Neo4j连接地址')
    username: str = Field(default_factory=lambda: os.getenv("NEO4J_USERNAME", "neo4j"), description='用户名')
    password: str = Field(default_factory=lambda: os.getenv("NEO4J_PASSWORD", "password"), description='密码')
...
class HotLayerConfig(BaseModel):
    """热层配置"""
...
class WarmLayerConfig(BaseModel):
    """温层配置"""
...
class ColdLayerConfig(BaseModel):
    """冷层配置"""
...
class MemoryConfig(BaseModel):
    ...
    neo4j: Neo4jConfig = Field(default_factory=Neo4jConfig, description='Neo4j配置')
    hot_layer: HotLayerConfig = Field(default_factory=HotLayerConfig, description='热层配置')
    warm_layer: WarmLayerConfig = Field(default_factory=WarmLayerConfig, description='温层配置')
...
class PrometheaConfig(BaseModel):
...
class Config:
    ...
    def __init__(self, **kwargs):
        ...
        if not self.api.api_key or self.api.api_key == 'placeholder-key-not-set':
            print('警告：API密钥未配置')
...
def load_config():
    ...
    if os.path.exists(config_path):
        try:
            ...
            return PrometheaConfig(**config_data)
