...
class HttpApiChannelAdapter(ChannelAdapter):
    metadata = ChannelMetadata(
        ...
        session_model="per_user",
    )

    def normalize_identity(self, raw_input: Dict[str, Any]) -> IdentityContext:
        user_id = str(raw_input.get("user_id") or "default_user")
        return IdentityContext(
            channel_id=self.metadata.channel_id,
            user_id=user_id,
            channel_user_id=user_id,
            attributes={"auth": "http_token"},
        )

    def permission_check(self, identity_context: IdentityContext) -> PermissionDecision:
        if identity_context.user_id in {"", "anonymous", "guest"}:
            return PermissionDecision(allowed=False, reason="http_identity_required")
    ...
    def build_session_key(self, raw_input: Dict[str, Any]) -> str:
        ...
        if explicit:
            return explicit
        return f"http_{raw_input.get('user_id') or 'default_user'}"
